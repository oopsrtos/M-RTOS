...
;                                            全局变量&函数
;********************************************************************************************************
    IMPORT  M_RTOS_CurPtr                                         ; 外部文件引人的参考
    IMPORT  M_RTOS_RdyPtr
		
    EXPORT  OSStartHighRdy                                      ; 该文件定义的函数
	EXPORT  PendSV_Handler
	EXPORT  m_rtos_set_interrupt_start
	EXPORT	m_rtos_set_interrupt_stop
;********************************************************************************************************
;                                               常量
;********************************************************************************************************
;--------------------------------------------------------------------------------------------------------
...
; 任务的保存，即把CPU寄存器的值存储到任务的堆栈中	
	...
	LDR     R1, = M_RTOS_CurPtr                 ; 加载 OSTCBCurPtr 指针的地址到R1，这里LDR属于伪指令
...
OS_CPU_PendSVHandler_nosave  
	...
	LDR     R0, = M_RTOS_CurPtr               ; 加载 OSTCBCurPtr 指针的地址到R0，这里LDR属于伪指令
	LDR     R1, = M_RTOS_RdyPtr               ; 加载 OSTCBHighRdyPtr 指针的地址到R1，这里LDR属于伪指令
